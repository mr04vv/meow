...
pub async fn create_request(
    ...
    conn.execute(
        "INSERT INTO requests (id, collection_id, name, method, url, headers, query_params, body, auth_type, auth_config, sort_order, created_at, updated_at)
...
pub async fn list_requests(
    ...
    let requests = if let Some(cid) = collection_id {
        let mut stmt = conn.prepare(
            "SELECT id, collection_id, name, method, url, headers, query_params, body, auth_type, auth_config, sort_order, created_at, updated_at
...
pub async fn update_request(
    ...
    conn.execute(
        "UPDATE requests SET collection_id=?1, name=?2, method=?3, url=?4, headers=?5, query_params=?6, body=?7, auth_type=?8, auth_config=?9, sort_order=?10, updated_at=?11
         WHERE id=?12",
        rusqlite::params![
            collection_id, name, method, url, headers_json, query_params_json, body,
            auth_type, auth_config_json, sort_order, now, request.id
        ],
    )?;
