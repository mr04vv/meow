...
### Added
- **Collection リポジトリ紐付け + Pull/Sync**: GitHub からインポートした Collection にリポジトリ情報（owner/repo/branch/ファイルパス）を保存。Collection 設定画面でブランチを選択して最新スペックを Sync 可能。user_edited フラグが立っているリクエストは Sync でスキップ
